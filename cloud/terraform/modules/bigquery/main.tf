...
resource "google_bigquery_table" "pipeline_runs" {
  ...
  schema = jsonencode([
    ...
    { name = "duplicate_keys",   type = "INT64",     mode = "NULLABLE" },
    { name = "quality_passed",   type = "BOOL",      mode = "NULLABLE" },
    { name = "null_violations",  type = "STRING",    mode = "REPEATED" },
    { name = "range_violations", type = "STRING",    mode = "REPEATED" },
